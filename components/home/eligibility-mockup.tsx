...
export function EligibilityMockup() {
  return (
    <>
      {/* Desktop: floating card above the lead form */}
      <div
        aria-hidden="true"
      ...
      {/* Mobile/tablet: compact inline strip, in-flow (not floating) above the form */}
      <div
        aria-hidden="true"
        className="anim-fade-in mb-4 flex items-center gap-2 rounded-xl border border-gold/25 bg-gold/10 px-3 py-2.5 lg:hidden"
      >
        <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gold text-gold-foreground">
          <CheckCircle2 className="h-4 w-4" aria-hidden="true" />
        </span>
        <div>
          <p className="font-serif text-sm leading-none text-foreground">100% זכאות מאושרת</p>
          <p className="text-xs text-muted-foreground">3,820 ₪ לחודש</p>
        </div>
      </div>
    </>

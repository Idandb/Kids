            ...
            {steps.map((step, i) => (
              <li key={step.num} className="h-full">
                <Reveal delay={i * 110} className="h-full">
                  <div className="card-lift relative flex h-full flex-col gap-2.5 rounded-2xl bg-card p-5 shadow-elevated">
                    <span
                      className="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-gold to-gold/70 font-serif text-base text-navy shadow-lg shadow-gold/25"
                      aria-hidden="true"
                    >
                      {step.num}
                    </span>

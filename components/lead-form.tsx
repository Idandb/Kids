...
const qualifiers: Partial<Record<string, { question: string; options: string[] }>> = {
  'פטור ממס הכנסה': {
    question: 'מה אחוז הנכות הרפואית שנקבע לך (אם כבר יש)?',
    options: ['90% ומעלה', '74%-89%', 'מתחת ל-74%', 'עדיין לא נקבע אחוז'],
  },
  'תאונת עבודה': {
    question: 'מתי קרתה הפגיעה?',
    options: ['בחודש האחרון', 'לפני 1-6 חודשים', 'לפני חצי שנה עד שנתיים', 'לפני יותר משנתיים'],
  },
  'קצבת נכות כללית': {
    question: 'מה המצב מול ביטוח לאומי כרגע?',
    options: ['עוד לא הגשתי תביעה', 'הגשתי וממתין/ה לתשובה', 'הוגשה ונדחתה', 'אושרה באחוז נמוך מהמגיע'],
  },
  'תביעת סיעוד': {
    question: 'עבור מי בודקים את הזכאות?',
    options: ['עבור עצמי', 'עבור הורה', 'עבור בן/בת זוג', 'עבור קרוב משפחה אחר'],
  },
  'הפרעות קשב (ADHD)': {
    question: 'מי מקבל/ת את האבחנה?',
    options: ['אני, כמבוגר/ת', 'הילד/ה שלי'],
  },
}
...
function buildWhatsAppUrl(name: string, phone: string, topic: string, qualifier: string) {
  ...
  const message = `שלום, אני ${name} ואשמח לבדיקת זכאות חינם בנושא: ${context}. אפשר לחזור אליי בטלפון ${phone}. תודה!`
...
export function LeadForm({
  variant = 'light',
  title = 'בדיקת זכאות חינם — עונים על כמה שאלות ומקבלים תשובה',
}: {
  variant?: 'light' | 'dark'
  title?: string
}) {
